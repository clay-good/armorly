/**
 * Pattern Recognition Library for Armorly
 *
...
class PatternLibrary {
  ...
  deobfuscate(text) {
    ...
    if (entityPattern.test(text)) {
      try {
        // DOMParser doesn't execute scripts, safer than innerHTML
        const parser = new DOMParser();
        const doc = parser.parseFromString(text, 'text/html');
        deobfuscated += '\n[DECODED]: ' + doc.documentElement.textContent;
      } catch (e) {
        // Fallback: manual entity decoding for common entities
        const entities = {
          '&lt;': '<', '&gt;': '>', '&amp;': '&', '&quot;': '"',
          '&#39;': "'", '&apos;': "'", '&nbsp;': ' '
        };
...
// Export for ES6 modules (background service worker)
